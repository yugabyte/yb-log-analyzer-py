            ...
            if file not in extractedFiles:
                ...
                with tarfile.open(file, "r:gz") as tar:
                    try:
                        ...
                        logger.warning(
                            "Got EOF Exception while extracting file {}, File might have still extracted. Please check {} for more information ".format(
                                file, log_file))
...
def analyzeLogFile(logFile, outputFile, logFilesMetadata, start_time, end_time):
    ...
    with logFileHandle as f:
        for line in f:
            ...
            if timeFromLog > end_time:
                logger.info(
                    "Reached end time: {}. Stopping analysis for file: {}".format(end_time.strftime("%m%d %H:%M"),
                                                                                  logFile))
                break
    ...
    if table:
        ...
        """
        content += tabulate.tabulate(table, headers=["Error Message", "Count", "First Occurrence", "Last Occurrence"],
                                     tablefmt="html")
        content = content.replace("$line-break$", "<br>").replace("$tab$", "&nbsp;&nbsp;&nbsp;&nbsp;").replace(
            "$start-code$", "<code>").replace("$end-code$", "</code>").replace("$start-bold$", "<b>").replace(
            "$end-bold$", "</b>").replace("$start-italic$", "<i>").replace("$end-italic$", "</i>").replace("<table>",
                                                                                                           "<table class='sortable' id='main-table'>")
...
def main(args=None):
    ...
    if args.support_bundle or args.directory:
        ...
        if start_time:
            ...
            includedLogFiles, removedFiles = filterLogFilesByTime(logFilesToProcess, logFilesMetadata, start_time,
                                                                  end_time)
            logFilesToProcess = [logFile for logFile in logFilesToProcess if logFile not in removedFiles]
            logger.debug(f"Filtered log files: {includedLogFiles}")
            logger.debug(f"Removed log files: {removedFiles}")
        ...
        for file in logFilesToProcess:
            table.append([file[-100:], logFilesMetadata[file]["logStartsAt"], logFilesMetadata[file]["logEndsAt"],
                          logFilesMetadata[file]["logType"], logFilesMetadata[file]["nodeName"]])
        table.sort(key=lambda x: (x[4], x[3], x[1]))  # Sort by Node Name, Type, then Start Time
        print(tabulate.tabulate(table, headers=["File", "Start Time", "End Time", "Type", "Node Name"],
                ...
                if nodeDetails:
                    for node, details in nodeDetails.items():
                        ...
                        for message, messageDetails in details.items():
                            ...
                            # Update count
                            nodeMessages[message]["count"] = nodeMessages[message].get("count", 0) + messageDetails.get(
                                "count", 0)
        ...
        if "pg" in choosenTypes:
            missingNodes = [node for node in nodes if not any(
                node in file for file in logFilesToProcess if logFilesMetadata[file]["logType"] == "postgres")]
            if missingNodes:
        ...
        if "ts" in choosenTypes:
            missingNodes = [node for node in nodes if not any(
                node in file for file in logFilesToProcess if logFilesMetadata[file]["logType"] == "yb-tserver")]
            if missingNodes:
        ...
        if "ms" in choosenTypes:
            missingNodes = [node for node in nodes if not any(
                node in file for file in logFilesToProcess if logFilesMetadata[file]["logType"] == "yb-master")]
            if missingNodes:
                print(
                    colorama.Fore.RED + f"Master logs missing for nodes: {', '.join(missingNodes)} - Ignore if these are not master nodes")
                isLogMissing = True
        # Controller logs
        if "ybc" in choosenTypes:
            missingNodes = [node for node in nodes if not any(
                node in file for file in logFilesToProcess if logFilesMetadata[file]["logType"] == "yb-controller")]
            if missingNodes:
        ...
        if isLogMissing:
            print(
                colorama.Fore.YELLOW + "WARNING: If missing logs are reported and if it is suspicious, please check the logs manually.")
        print("=====================================")
